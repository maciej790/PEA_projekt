...
#include <iostream>
using namespace std;
using namespace std::chrono; // Ułatwia korzystanie z chrono
...
int tsp(int odwiedzone[], int pozycja, int liczbaOdwiedzonych) {
  // Jeśli odwiedziliśmy wszystkie wierzchołki, wracamy do początkowego
  // wierzchołka 0
  if (liczbaOdwiedzonych == n) {
    // Sprawdzamy, czy istnieje połączenie powrotne do wierzchołka startowego
    // (0)
  ...
  // Próbujemy przejść do każdego wierzchołka, który nie został jeszcze
  // odwiedzony
  for (int next = 0; next < n; next++) {
    if (!odwiedzone[next] &&
        graf[pozycja][next] >
            0) { // Sprawdzamy, czy wierzchołek next nie został odwiedzony
      odwiedzone[next] = 1; // Zaznaczamy wierzchołek next jako odwiedzony
      int tempCost =
          tsp(odwiedzone, next,
              liczbaOdwiedzonych +
                  1); // Rekurencyjne wywołanie TSP dla nowego wierzchołka
      if (tempCost != INT_MAX) {
        minCost = min(minCost,
                      graf[pozycja][next] + tempCost); // Minimalizujemy koszt
      }
...
int main() {
  ...
  int *odwiedzone =
      new int[n]();  // Alokujemy tablicę i inicjalizujemy ją zerami
  ...
  int wynik =
      tsp(odwiedzone, 0,
          1); // Startujemy od wierzchołka 0 z jednym odwiedzonym wierzchołkiem
